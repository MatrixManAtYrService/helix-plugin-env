...
(provide create-gs-picker add-modified-file)

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(provide scheme-indent)

(define (git-add) (shell "git" "add" "%"))

(require "cogs/recentf.scm")
(provide refresh-files flush-recent-files recentf-snapshot recentf-open-files)
...
(define (select-random lst)
  (let ((index (rand::rng->gen-range rng 0 (length lst))))
    (list-ref lst index)))
...
(define (move-window-left) (helix.static.move-window-far-left))
...
(define (helix-picker! . pick-list) (push-component! (picker pick-list)))

;; I think options might still come through as void?
(define (unwrap-or obj alt) (if (void? obj) alt obj))
...
(define (mark-last-focused!)
  (let* ((focus (editor-focus))) (set! *last-focus* focus) focus))

(define (currently-focused) (editor-focus))

;; (hash? string? )
; (define *temporary-buffer-map* (hash))
; (set! *reverse-buffer-map* (hash))
;; Grab whatever we're currently focused on
(define (get-current-focus) (~> (editor-focus)))
    ...
    ((minor-mode-cruncher (key => (function ...)))
     (hash key
           (map (lambda (x) (string-append ":" (symbol->string x)))
                '(function ...))))
    ((minor-mode-cruncher (key => function))
     (hash key (string-append ":" (symbol->string 'function))))
    ((minor-mode-cruncher (key => (function ...)) remaining ...)
     (hash-insert
...
(define (git-status) (helix.run-shell-command "git" "status"))
...
;;@doc run git status
(define (new-function) (git-status))

;;@doc
;; Collect memory usage of engine runtime?
(define (print-engine-stats) (error "TODO"))
...
(skip-compile
  ...
  (define fail-check 0)
  (define (reset-fail-check!) (set! fail-check 0))
  (define (mark-failed!) (set! fail-check (+ 1 fail-check)))
  (define (increase-terminal-refresh-delay!)
    (set! *terminal-refresh-delay*
      (* *terminal-refresh-delay* *terminal-refresh-delay*)))
  ...
  (define (kill-terminal) (kill-pty-process! *pty-process*))
  (define *go-signal* #t)
  (provide interrupt-terminal)
  (define (interrupt-terminal)
    (set! *go-signal* #f)
    (enqueue-thread-local-callback (lambda () (set! *go-signal* #t rue))))
...
(skip-compile
  (define (read-until-no-more-lines) (error! "TODO"))
  (define fail-check 0)
...
(skip-compile
  ...
  (define escape-code-map
    (list (lambda (cx) (when (equal? 1 *cursor-position*) (helix-clear-line)))
          helix.static.kill_to_line_start
          (lambda ()
            (helix.static.extend_to_line_bounds)
            (helix.static.delete_selection))
          (lambda ()
            (helix.static.insert_string "CURSOR POSITION ESCAPE SEQUENCE"))))
  (define (async-write-from-terminal-loop line)
    (unless (hash-try-get *temporary-buffer-map* "steel-repl") (open-repl))
    (temporarily-switch-focus
      (lambda ()
        (open-labelled-buffer "steel-repl")
        (transduce
          (tokenize-line *ansi-parser* line)
          (into-for-each
            (lambda (line)
              (cond ((int? line) ((list-ref escape-code-map line) cx))
  ...
  (define (send-command . args)
    ...
    (pty-process-send-command
      *pty-process*
      (apply string-append carriage-return-ammended-string))))
...
(define-syntax
  ...
  (syntax-rules
    ...
    ((skip-compile expr exprs ...) (begin (skip-compile exprs ...)))))
